...
import net.anweisen.utilities.jda.commandmanager.impl.parser.*;
...
public class DefaultParserContext implements ParserContext {
	...
	public DefaultParserContext() {
		...
		registerParser("guild:category",        Category.class,         new GuildRelatedArgumentParser<>(SearchHelper::findCategory));
		registerParser("guild:channel",         GuildChannel.class,     new GuildRelatedArgumentParser<>(SearchHelper::findGuildChannel));
		registerParser("guild:textchannel",     TextChannel.class,      new GuildRelatedArgumentParser<>(SearchHelper::findTextChannel));
		registerParser("guild:voicechannel",    VoiceChannel.class,     new GuildRelatedArgumentParser<>(SearchHelper::findVoiceChannel));
		registerParser("guild:role",            Role.class,             new GuildRelatedArgumentParser<>(SearchHelper::findRole));
		registerParser("bot:user",              User.class,             new BotRelatedArgumentParser<>(SearchHelper::findUser, SearchHelper::findUser));
	}
